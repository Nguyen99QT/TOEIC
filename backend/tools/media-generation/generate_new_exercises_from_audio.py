...
def get_image_files() -> List[str]:
    ...
    def extract_num(x: str) -> int:
        m = re.search(r'ex(\d+)\.jpg', x)
        return int(m.group(1)) if m and m.group(1) else 0
    return sorted(image_files, key=extract_num)
...
def generate_exercise_sql(exercise_id: int, title: str, description: str, difficulty: str) -> str:
...
def generate_question_sql(exercise_id: int, question_order: int, audio_num: int, question_type: str, difficulty: str) -> str:
