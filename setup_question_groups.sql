-- Manual SQL to create QuestionGroups with content for Part 6 and 7

-- First, let's see what parts exist
SELECT * FROM parts WHERE part_number IN (6, 7);

-- Insert Part 6 Question Group
...
Best regards,
Regional Manager',
    (SELECT part_id FROM parts WHERE part_number = 6 LIMIT 1),
    1  -- Assuming admin user has ID 1
);

-- Insert Part 7 Question Group
INSERT INTO question_groups (title, description, content, part_id, created_by)
VALUES (
...
Human Resources Department',
    (SELECT part_id FROM parts WHERE part_number = 7 LIMIT 1),
    1  -- Assuming admin user has ID 1
);

-- Now update question_test records to link them to the groups
...
SELECT 
    ...
    LENGTH(qg.content) as content_length
