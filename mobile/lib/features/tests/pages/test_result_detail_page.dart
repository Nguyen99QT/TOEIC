...
class _TestResultDetailPageState extends ConsumerState<TestResultDetailPage> {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        leading: IconButton(
          ...
          onPressed: () {
            // Try context.pop() first, fallback to Navigator if that fails
            try {
              if (context.canPop()) {
                context.pop();
              } else {
                // Fallback: navigate to test history
                context.go('/test-history');
              }
            } catch (e) {
              // Double fallback: use Navigator
  ...
  Widget _buildQuestionImage(String imageUrl) {
    return Container(
      ...
      child: QuestionImageWidget(imageUrl: imageUrl),
    );
  }

  Widget _buildQuestionAudio(String audioUrl) {
    print('Result Detail - Audio URL: $audioUrl'); // Debug log
    return Container(
      margin: const EdgeInsets.symmetric(vertical: 8),
