...
import { contactService, ContactRequest, contactUtils } from '../../services/contact';
...
const ContactForm: React.FC<ContactFormProps> = ({ onSuccess, onCancel }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        toast.success('Gửi contact thành công!');
        setFormData({
          subject: '',
          content: '',
          contactType: 'GENERAL',
      ...
      toast.error('Có lỗi xảy ra khi gửi contact');
    } finally {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <input
            ...
            placeholder="Nhập tiêu đề contact..."
            required
          />
        </div>
        ...
        <div className="bg-gray-50 p-4 rounded-md">
          <div className="flex items-center space-x-2">
            <div 
              className={`w-4 h-4 rounded-full bg-${contactUtils.getPriorityColor(formData.priority)}-500`}
            ></div>
            <span className="text-sm text-gray-600">
              Độ ưu tiên: <span className="font-medium">{contactUtils.formatPriority(formData.priority)}</span>
            </span>
          </div>
          <div className="mt-2 text-xs text-gray-500">
            Loại: <span className="font-medium">{contactUtils.formatContactType(formData.contactType)}</span>
          </div>
        </div>
